...
$method = $_SERVER['REQUEST_METHOD'];
  ...
  if ($nueva_consulta = $mysqli->prepare("SELECT *  FROM usuarios WHERE dpi = ?")) {
        ...
        if ($resultado->num_rows == 1) {
            ...
            if ($pas == $encriptado_db)
            {
                ...
                echo json_encode(array('conectado'=>true,'dpi'=>$datos['dpi'], 'nombre'=>$datos['nombre'],  'apellido'=>$datos['apellido'], 'correo'=>$datos['correo'],'clave'=>$datos['clave'], 'fecha_de_nacimiento'=>$datos['fecha_de_nacimiento'] ,'enfermedad'=>$datos['enfermedad'], 'celular'=>$datos['celular'], 'tipoUsuario'=>$datos['tipoUsuario'] ) );
              }
